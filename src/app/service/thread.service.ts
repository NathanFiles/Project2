...
export class ThreadService {
  ...
  getThread(param) {
    this.threadnum = param.id;
    for (let i=0; i<this.threads.length; i++) {
      if (this.threads[i].id == this.threadnum && this.threads[i].active) {
        for (let j=0; j<this.posts.length; j++) {
          if (this.posts[j].t_id==this.threads[i].id && this.posts[j].parent_id==null) {
            console.log("HIT");
  ...
  addThread(threadVar: thread) : Observable<thread> {
    return this.http.post<thread>("http://localhost:8080/threads", threadVar, {headers: this.headers});
  }
}
